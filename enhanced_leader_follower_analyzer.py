...
class EnhancedLeaderFollowerAnalyzer:
    ...
    def generate_comprehensive_report(self, pairs_df, stats):
        ...
        print("詳細報告已保存至: leader_follower_analysis_report.txt")
        return report_text
    ...
    def create_visualizations(self, pairs_df, stats):
        ...
        print("圖表已保存至: leader_follower_comprehensive_analysis.png")
                        ...
                        buttons=[
                            ...
                            dict(label="8046 + 3189 Pair", 
                                 ...
                                 args=[{"visible": self._create_visibility_for_stocks(['8046', '3189'], fig)},
                                       {"title": f"8046→3189 Leader-Follower Pair - {date}"}]),
                            dict(label="8046 + 3037 Pair", 
                                 method="update",
                                 args=[{"visible": self._create_visibility_for_stocks(['8046', '3037'], fig)},
                                       {"title": f"8046→3037 Leader-Follower Pair - {date}"}]),
    ...
    def _create_visibility_for_stocks(self, target_stocks, fig):
        ...
        for trace in fig.data:
            ...
            if hasattr(trace, 'name') and trace.name:
                trace_name = trace.name
                # 檢查trace名稱中是否包含目標股票代碼
                for code in target_codes:
                    if code in trace_name:
                        should_show = True
                        break
                # 對於信號點，檢查hovertemplate中的股票代碼
    ...
    def save_detailed_results(self, pairs_df):
        ...
        if not pairs_df.empty:
            ...
            print(f"詳細結果已保存:")
            print(f"- leader_follower_pairs_detailed.csv ({len(pairs_df)} 筆記錄)")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("使用方法: python enhanced_leader_follower_analyzer.py <CSV檔案路徑>")
        print("例如: python enhanced_leader_follower_analyzer.py BigBuySell-min.csv")
        return
